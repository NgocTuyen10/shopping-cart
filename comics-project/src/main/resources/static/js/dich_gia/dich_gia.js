...
$(document).ready(function () {
  ...
  function loadDataTable() {
    var url = "/comics/management/list-dich-gia";
    disableButton();
  ...
  $("#edit").on('click', function () {
    ...
    var projectUrl = "/comics/management/dich-gia/" + id;
  ...
  $("#delete").on('click', function () {
    ...
    deleteOK.addEventListener('click', function () {
      ...
      console.log(idJson);
      $.ajax({
        url: '/comics/management/dich-gia/delete', // url where to submit the request
        type: "PUT", // type of action POST || GET
        contentType: "application/json", // data type
        data: idJson, // post data || get data
        success: function (result) {
          $('#delete-dich-gia-success').modal('show');
          idList = [];
          loadDataTable();
        },
        error: function (xhr, resp, text) {
          console.log(xhr, resp, text);
          $('#delete-dich-gia-error').modal('show');
        }
      });
...
function renderData(data) {
  var table = $('#datatable').DataTable({
    ...
    "pageLength ": 10,
    columns: [
      { searchable: false, title: "<input type='checkbox' class='checkbox' name='select_all' id='checkbox-select-all' onClick='checkAll()'>", data: 'dichGiaId' },
      { title: "Họ tên", data: 'ten' },
      { title: "Mã dịch giả", data: 'maDichGia' },
